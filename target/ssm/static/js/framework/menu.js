...
*
*/
function initSiderBar(token) {
    var data = testMenu;
    $.ajax({
        type: "POST",
        url: webProjRoot + "login/getmenu.action",
        async: false,
        headers: {
            token: token
        },
        //data: {token : token, systemId: systemId},
        dataType: "json",
...
function addPageIfm(menuid, url) {
    ...
    if (ifm.attachEvent) {
        ...
        ifm.onload = function () {
            ifm.style.height = getContentHeight() + "px";
            //$(".page-container").css("margin-top", "72px");
        };
    }
    ifm.src = url;
...
function processBreadcrumb(menuObj) {
    putPreMenuids(menuObj.getAttribute("menuid"));
...
function breadcrumbClick(menuid) {
    if (menuItems[menuid]) {
        putPreMenuids(menuid);
        menuItems[menuid].click();
    }
}

function attachMainIfmPage(url) {
    var ifm = $(".main_content_ifm")[0];
    ifm.setAttribute("frameborder", "0");
    if (ifm.attachEvent) {
        ifm.attachEvent("onload", function () {
...
function putPreMenuids(menuid) {
    if (!preMenuids[0]) {
        preMenuids[0] = menuid;
    } else {
        preMenuids[1] = preMenuids[0];
        preMenuids[0] = menuid;
    }
}

function getPreMemuid() {
    if (!!preMenuids[1]) {
        return preMenuids[1];
    } else if (!!preMenuids[0]) {
        return preMenuids[0];
    } else {
        return undefined;
    }
...
function preBack() {
    if (!!getPreMemuid() && menuItems[getPreMemuid()]) {
        menuItems[getPreMemuid()].click();
    }
}
