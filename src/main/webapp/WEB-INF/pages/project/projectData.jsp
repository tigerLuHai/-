...
<div id="orderlistWraper">
    <div style="padding:3px">
        ...
        <a href="javascript:void(0);" class="easyui-linkbutton" plain="true" onclick="logout()"
           style="width:70px;height:30px" id="btnadd">退出登录</a>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#orderList tbody').live('dblclick', 'tr', function () {
            ...
            if (!!rows && !!rows[0]) {
                var row = rows[0];
                window.location.href = 'projectClass?teacher1=' + row.teacher;
                return;
            } else {
    ...
    /* function classdata(){
         ...
         if (!!rows && !!rows[0]){
             var row = rows[0];
             window.location.href ='adminClass?teacher1='+row.teacher;
             return;
         } else {
             window.parent.showDiag("请选择一个用户", "noexec");
         }
    ...
    /* function studentsdata(){
         ...
         if (!!rows && !!rows[0]){
             var row = rows[0];
             window.location.href ='adminStudents?teacher='+row.teacher;
             return;
         } else {
             window.parent.showDiag("请选择一个用户", "noexec");
         }
